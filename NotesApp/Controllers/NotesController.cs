...
public class NotesController : ControllerBase {
    ...
    public async Task<ActionResult<ApiResponse<NoteResponseDto>>> Create(
        ...
        if (!await _categoryRepo.ExistsAsync(dto.CategoryId))
            return BadRequest(ApiError.BadRequest("Ошибка валидации", new List<string> { $"Категория с id={dto.CategoryId} не существует" }));
        var note = new Note {
    ...
    public async Task<ActionResult<ApiResponse<NoteResponseDto>>> Update(
        ...
        if (note is null)
            return NotFound(ApiError.NotFound($"Заметка с id = {id} не найдена"));
        if (!await _categoryRepo.ExistsAsync(dto.CategoryId))
            return BadRequest(ApiError.BadRequest("Ошибки валидации", new List<string> { $"Категория с id={dto.CategoryId} не существует" }));
